...
public class PaymentService {
     ...
     * 포트원 서버에 직접 조회(Server to Server) 한번 더 요청 (위변조 방지)
     *
     *
     *
     */
    ...
    public PaymentResponse.CompleteDTO 결제검증후포인트충전(Integer userId, String paymentId) {
        ...
        // 1. 사전 검증 (실제 사용자가 맞는지 (userId, paymentId DB에 이미 존재 하는지 확인)
        User userEntity = userRepository.findById(userId).orElseThrow(() ->
                new Exception404("사용자를 찾을수 없습니다"));

        if (paymentRepository.existByPaymentId(paymentId)) {
       ...
       PaymentResponse.PortOnePayment portOnePayment = 포트원단건결제조회(paymentId);
        ...
        if (portOnePayment.getAmount() == null || portOnePayment.getAmount().getTotal() == null) {
            throw new Exception400("결제 금액 정보를 확인할수 없습니다");
        }
