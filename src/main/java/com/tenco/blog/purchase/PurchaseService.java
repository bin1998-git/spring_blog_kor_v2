...
public class PurchaseService {
     ...
     * 유료 게시글 구매 로직
     * <p>
     * 트랜 잭션 처리 : 포인트 차감과 구매 내역(저장)을 하나의 트랜잭션으로 처리
     *
     *
     */
    // 유료 게시글 구매
    ...
    public User 구매하기(Integer userId, Integer boardId) {
        ...
        // 3. 작성자가 자신의 게시글을 구매 하려는 경우는 방지
        if (boardEntity.getUser() != null && boardEntity.getUser().getId().equals(userId)) {
            throw new Exception400("자신이 작성한 게시글은 구매할수 없습니다");
        }
        ...
        // controller 단에서 차감된 포인트 상태를 위해 세션 동기화 처리 (반환)
